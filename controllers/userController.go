...
func Signup() gin.HandlerFunc{
	return func (c *gin.Context) {
		...
		if count > 0 {
			c.JSON(http.StatusInternalServerError, gin.H{"error":"This email or phone number doesnot exist"})

		}
        
		user.Created_at, _ = time.Parse(time.RFC3339, time.Now().Format(time.RFC3339))
		user.Updated_at, _ = time.Parse(time.RFC3339, time.Now().Format(time.RFC3339))
		user.ID = primitive.NewObjID()
...
func Login() gin.HandlerFunc{
		...
		userCollection.FinOne(ctx, bson.M{"email":user.Email}).Decode(&foundUser)
		defer cancel()
		err != nil{
			c.JSON(http.StatusInternalServerError, gin.H{"error":"email or passowrd is not valid"})
			return
		}
		...
		if passwordIsValid != true{
			c.JSON(http.Status.StatusInternalServerError, gin.H{"error":msg})
			return
		}
	...
	return func (c *gin.Context)
	  ...
	  projectStage := bson.D{
		  {"$project", bson.D{
			  {"_id", 0},
		{"total_count", 1},
		{"user_items", bson.{{"$slice", []interface{}{"$data", recordPerPage}}}},
