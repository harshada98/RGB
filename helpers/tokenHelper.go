...
func GenerateAllTokens(email string, firstName string, lastName string, userType string, uid string) (signedToken string, signedRefreshToken string) {
	...
	refreshToken, err := jwt.NewWithClaims(jwt.SigningMethodES256, claims).refreshClaims([]byte(SECRET_KEY))
...
func UpdateAllTokens(signedToken string, signedRefreshToke string, UserId string) {
